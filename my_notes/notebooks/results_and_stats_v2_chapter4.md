This is a markdown file, now. the folllowing following describe a notebook.
 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "\n",
    "**Objective**: Rigorous statistical validation of GPU-accelerated GA variants against CPU baselines.\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "ALGORITHMS = [\"GeneticAlgorithmCPU\", \"GeneticAlgorithmHybridNaive\", \"GeneticAlgorithmHybridOptimized\", \"GeneticAlgorithmFullGPU\"]\n",
    "SHORT_NAMES = {\n",
    ...
    "}\n",
    "COLORS = {\"CPU\": \"#e74c3c\", \"HybridNaive\": \"#3498db\", \"HybridOpt\": \"#2ecc71\", \"FullGPU\": \"#9b59b6\"}\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "df[\"Category\"] = pd.cut(df[\"Size\"], bins=[0, 100, 500, 2000], labels=[\"Small\", \"Medium\", \"Large\"])\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "Aggregated metrics across all problem sizes."
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    "## 2. Speedup Analysis\n",
    "**Baseline**: `HybridNaive` (available for all problem sizes).\n",
    "**Metric**: $S = T_{HybridNaive} / T_{Variant}$."
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
  ...
  },
    {
     ...
     "source": [
        ...
        "- **Speedup vs problem size**: $S^{CPU}_{p,a} = T_{CPU,p} / T_{a,p}$.\\n",
        "- **Distribution of speedups** across all small problems.\\n",
        "\\n",
        "A value of $S^{CPU}_{p,a} = 2$ means that the GPU algorithm is twice as fast as the CPU on problem $p$. Values below 1 indicate that the GPU algorithm is slower than the CPU.\\n"
     ]
    },
    {
     "cell_type": "code",
     ...
     "source": [
        ...
        "gpu_small = df_small[df_small[\"Algorithm\"].isin([\"HybridNaive\", \"HybridOpt\", \"FullGPU\"]) & df_small[\"SpeedupCPU\"].notna()]\\n",
        "\\n",
        "fig, axes = plt.subplots(1, 2, figsize=(18, 7))\\n",
        "\\n",
        "# 1. Speedup vs Size (CPU baseline)\\n",
        "sns.scatterplot(data=gpu_small, x=\"Size\", y=\"SpeedupCPU\", hue=\"Algorithm\", style=\"Algorithm\",\\n",
        "                s=100, palette=COLORS, ax=axes[0])\\n",
        ...
        "sns.boxplot(data=gpu_small, x=\"Algorithm\", y=\"SpeedupCPU\", palette=COLORS, ax=axes[1])\\n",
        "axes[1].axhline(1.0, color=\"k\", linestyle=\"--\")\\n",
    ...
    },
  {
   ...
   "source": [
    ...
    "Visualizing the trade-off between execution speed and solution quality. Ideally, we want points in the bottom-left corner (Low Time, Low Gap)."
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "sns.scatterplot(data=df, x=\"Time\", y=\"Gap\", hue=\"Algorithm\", style=\"Category\", size=\"Size\", sizes=(50, 400), palette=COLORS, alpha=0.7)\n",
    "plt.xscale(\"log\")\n",
  ...
  },
   {
    ...
    "source": [
      "### 3.1 Quality–Speed Tradeoff per Problem\\n",
      "\\n",
      ...
      "  - the speedup $S_{p,a} = T_{base,p} / T_{a,p}$.\\n",
      "\\n",
   ...
   },
  {
   ...
   "source": [
    ...
    "Using **Violin Plots** to see the full shape of the data distributions and **CV** to measure stability."
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "\n",
    "# Violin: Time\n",
    "sns.violinplot(data=df, x=\"Algorithm\", y=\"Time\", hue=\"Algorithm\", ax=axes[0,0], palette=COLORS, legend=False)\n",
    "axes[0,0].set_yscale(\"log\")\n",
    "axes[0,0].set_title(\"Runtime Distribution (Log Scale)\")\n",
    "\n",
    "# Violin: Gap\n",
    "sns.violinplot(data=df, x=\"Algorithm\", y=\"Gap\", hue=\"Algorithm\", ax=axes[0,1], palette=COLORS, legend=False)\n",
    "axes[0,1].set_title(\"Optimality Gap Distribution\")\n",
    "\n",
    "# Scatter: CV vs Size\n",
    "df[\"CV\"] = df[\"StdTime\"] / df[\"Time\"]\n",
    "sns.scatterplot(data=df, x=\"Size\", y=\"CV\", hue=\"Algorithm\", style=\"Algorithm\", palette=COLORS, ax=axes[1,0])\n",
    "axes[1,0].set_xscale(\"log\")\n",
    "axes[1,0].set_title(\"Runtime Stability (CV) vs Problem Size\")\n",
    "axes[1,0].set_ylabel(\"Coefficient of Variation (Std/Mean)\")\n",
    "\n",
    "# Boxplot: Generations\n",
    "sns.boxplot(data=df, x=\"Algorithm\", y=\"Generations\", hue=\"Algorithm\", palette=COLORS, ax=axes[1,1])\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "sns.heatmap(matrix_time, annot=True, xticklabels=algos_comp, yticklabels=algos_comp, cmap=\"RdBu_r\", center=0, ax=axes[0])\n",
    "axes[0].set_title(\"Cohen's d: Runtime (Red = Row Slower)\")\n",
    "\n",
    "sns.heatmap(matrix_gap, annot=True, xticklabels=algos_comp, yticklabels=algos_comp, cmap=\"RdBu_r\", center=0, ax=axes[1])\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    \n",
    "    radar_data.append({\"Algorithm\": algo, \"Speed\": speed_score, \"Quality\": quality_score, \"Stability\": stability_score})\n",
    "\n",
    ...
    "    values += values[:1]\n",
    "    ax.plot(angles, values, linewidth=2, linestyle='solid', label=row[\"Algorithm\"], color=COLORS[row[\"Algorithm\"]])\n",
